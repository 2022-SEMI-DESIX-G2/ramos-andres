(() => {
    const App = {
      ...
      handlers: {
        handleFormSubmit: async (e) => {
          ...
          try {
            ...
            App.htmlElements.response.innerHTML = App.templates.pokemonCard(data.data);
            if (data.data.sprites){
              App.htmlElements.response.innerHTML += App.templates.spriteCard(data.data); 
            }
            if (data.data.evoList){
      ...
      evolutionCard: ({evoList}) => {
          const evolutionList = evoList.map(
              (name) => 
              `<li style="text-transform: capitalize">${name.name}</li>`
      ...
      locationCard: ({locations}) => {
        const locationList = locations.map(({ location_area, version_details }) => {
          let counter = 0
          ...
          counter++;
          return `<li>${location} <br>
            Chance: ${chance}</li>`
      });
        // ubications = locations.map(
        //   (locations) =>
        ...
        return '<div class="location-card">'+
                  '<div class="pokemon-location">'+
                    '<p><b>Location Encounters</b></p>'+
                    `<ul>${locationList.join("")}</ul>`+
                  '</div>'+  
                '</div>';
      },
